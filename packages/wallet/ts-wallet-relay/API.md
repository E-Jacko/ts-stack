...
| `onSocketClosed`        | `(info: SocketCloseInfo) => void` | No   | —                                               | Called for every accepted WebSocket that closes, both roles, with the close code, the cause (`client`, `heartbeat` or `server`) and how long it was open. Fires before the session bookkeeping that drives `onSessionDisconnected`. Intended for logging, so a dropped phone can be diagnosed from server logs. |
| `heartbeatIntervalMs`   | `number`                      | No       | `30000`                                         | How often to ping every socket, in milliseconds. Must be an integer >= 1; forwarded to `WebSocketRelay`. |
| `maxMissedHeartbeats`   | `number`                      | No       | `2`                                             | Consecutive missed pongs tolerated before a socket is terminated. Must be an integer >= 1; set `1` for the pre-0.5 terminate-on-first-miss behaviour. Forwarded to `WebSocketRelay`. |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
interface WebSocketRelayOptions {
  ...
  heartbeatIntervalMs?: number      // ping period in ms, integer >= 1 (default 30000)
...
Called for every accepted socket that closes, whether or not it still held its topic slot. Unlike `onDisconnect` this also fires for a socket that was replaced by a newer connection on the same topic and role. Intended for logging.
