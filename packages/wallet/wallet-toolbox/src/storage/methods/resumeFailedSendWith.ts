...
export async function resumeFailedSendWith(storage: StorageProvider, userId: number, txids: string[]): Promise<void> {
  for (const txid of new Set(txids)) {
    ...
    const beef = Beef.fromBinary(original.inputBEEF)
    beef.mergeRawTx(original.rawTx)
    ...
    const preparedOutputs = await storage.findOutputs({ partial: { transactionId: preparedAction.transactionId, userId } })
    const preparedInputs = (await Promise.all(raw.inputs.map(input => storage.findOutputs({
    ...
    await storage.transaction(async trx => {
      ...
      if (JSON.stringify(req.rawTx) !== JSON.stringify(original.rawTx) || JSON.stringify(req.inputBEEF) !== JSON.stringify(original.inputBEEF))
      ...
      if (transactions.length !== 1 || transactions[0].userId !== userId || !transactions[0].isOutgoing ||
      ...
      if (action.transactionId !== preparedAction.transactionId) throw new WERR_INVALID_OPERATION('Exact retry action changed.')
      ...
      const outputs = await storage.findOutputs({ partial: { transactionId: action.transactionId, userId }, trx })
      for (const output of outputs) {
        const signed = raw.outputs[output.vout]
        if (!lockedOutputIds.has(output.outputId) || !signed || output.txid !== txid || output.satoshis !== signed.satoshis || output.spentBy != null ||
            (output.lockingScript != null && JSON.stringify(output.lockingScript) !== JSON.stringify(signed.lockingScript.toBinary())))
          throw new WERR_INVALID_OPERATION('Exact retry output lineage is inconsistent or allocated.')
      }
      for (const input of raw.inputs) {
        const matches = await storage.findOutputs({ partial: { userId, txid: input.sourceTXID, vout: input.sourceOutputIndex }, trx })
        ...
        const locked = output
        if ((locked.spentBy != null && locked.spentBy !== action.transactionId) || (!locked.spendable && locked.spentBy !== action.transactionId))
      ...
      req.attempts = 0
      req.notified = false
      req.addHistoryNote({ what: 'exactSendWithResume', when: new Date().toISOString(), transactionId: action.transactionId })
