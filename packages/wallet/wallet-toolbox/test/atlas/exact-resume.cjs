...
async function main () {
  let walletDb = knex({ client: 'better-sqlite3', connection: { filename: join(mkdtempSync(join(tmpdir(), 'atlas-exact-wallet-')), 'wallet.sqlite') }, useNullAsDefault: true })
  ...
  let active = new StorageKnex({
    chain: 'test', knex: walletDb, commissionSatoshis: 0,
    commissionPubKeyHex: undefined, feeModel: { model: 'sat/kb', value: 1 }
  })
  ...
  let monitor = new Monitor({
    chain: 'test', storage: manager, services, chaintracks: services.tracker,
    msecsWaitPerMerkleProofServiceReq: 0, taskRunWaitMsecs: 0,
    abandonedMsecs: 300000, unprovenAttemptsLimitTest: 100,
    unprovenAttemptsLimitMain: 144, maxRebroadcastAttempts: 0,
    startupTaskMode: 'none'
  })
  monitor.addDefaultTasks()
  let wallet = new Wallet({
    chain: 'test', keyDeriver: new CachedKeyDeriver(root), storage: manager,
    services, monitor
  })
  ...
  try {
    ...
    const [coinbase] = await services.storage.knex('mockchain_utxos').where({ isCoinbase: true, blockHeight: 0, spentByTxid: null })
    const inputBeef = await services.getBeefForTxid(coinbase.txid)
    const action = new Transaction()
    action.addInput({ sourceTXID: coinbase.txid, sourceOutputIndex: 0, unlockingScript: Script.fromHex(''), sequence: 0xffffffff })
    ...
    const now = new Date()
    const txId = await active.insertTransaction({ created_at: now, updated_at: now, transactionId: 0, userId: user.userId, status: 'failed', reference: Buffer.from('exact-resume').toString('base64'), isOutgoing: true, satoshis: -1, description: 'exact resume regression', txid })
    const reqId = await active.insertProvenTxReq({ created_at: now, updated_at: now, provenTxReqId: 0, status: 'invalid', attempts: 1, notified: false, txid, history: JSON.stringify({ notes: [] }), notify: JSON.stringify({ transactionIds: [txId] }), rawTx, inputBEEF: inputBeef.toBinary() })
    const fundingId = await active.insertTransaction({ created_at: now, updated_at: now, transactionId: 0, userId: user.userId, status: 'completed', reference: Buffer.from('source').toString('base64'), isOutgoing: false, satoshis: 5_000_000_000, description: 'source fixture', txid: coinbase.txid })
    const outputBase = { created_at: now, updated_at: now, outputId: 0, userId: user.userId, change: false, outputDescription: 'test owned output', providedBy: 'you', purpose: '', type: 'custom' }
    const inputId = await active.insertOutput({ ...outputBase, transactionId: fundingId, spendable: true, vout: 0, satoshis: 5_000_000_000, txid: coinbase.txid, lockingScript: [81] })
    for (const [vout, output] of action.outputs.entries()) await active.insertOutput({ ...outputBase, transactionId: txId, spendable: false, vout, satoshis: output.satoshis, txid, lockingScript: [...output.lockingScript.toBinary()] })
    const scenario = process.argv[2] ?? 'normal' 
    const post = services.postBeef.bind(services)
    if (scenario === 'lost-response') services.postBeef = async (...args) => { await post(...args); throw new Error('provider response lost') }
    if (scenario === 'already-known') { const b = Beef.fromBinary(inputBeef.toBinary()); b.mergeRawTx(rawTx); await post(b, [txid]) }
    const resume = () => wallet.createAction({ description: 'resume exact signed action', options: { sendWith: [txid], acceptDelayedBroadcast: false } })
    ...
    if (scenario === 'output-mismatch') await active.updateOutput((await active.findOutputs({ partial: { transactionId: txId } }))[0].outputId, { satoshis: 22 })
    if (scenario === 'reallocated') await active.updateOutput(inputId, { spendable: false, spentBy: fundingId })
    if (['malformed', 'wrong-id', 'foreign-owner', 'output-mismatch', 'reallocated'].includes(scenario)) {
    ...
    if (scenario === 'prequeue-crash') {
      ...
      active.updateTransaction = async (id, fields, trx) => { if (id === txId && fields.status === 'sending') throw new Error('crash before queue commit'); return update(id, fields, trx) }
      await assert.rejects(resume)
      assert.equal((await active.findProvenTxReqs({ partial: { provenTxReqId: reqId } }))[0].status, 'invalid')
    ...
    if (scenario === 'postqueue-crash') {
      ...
      active.attemptToPostReqsToNetwork = async () => { throw new Error('crash after queue commit') }
      await assert.rejects(resume)
    ...
    if (scenario === 'status-commit-crash') {
      const update = active.updateTransaction.bind(active)
      active.updateTransaction = async (id, fields, trx) => { if (id === txId && fields.status === 'unproven') throw new Error('crash after post before status commit'); return update(id, fields, trx) }
      await assert.rejects(resume)
      assert.equal((await active.findProvenTxReqs({ partial: { provenTxReqId: reqId } }))[0].status, 'unsent')
    ...
    if (scenario === 'provider-unknown' || scenario === 'malformed-status') {
      services.postBeef = async () => { throw new Error('offline') }
      if (scenario === 'malformed-status') services.getStatusForTxids = async () => ({ status: 'success', results: [{ txid: 'ff'.repeat(32), status: 'known' }] })
      const pending = await resume().catch(error => { assert.equal(error.name, 'WERR_REVIEW_ACTIONS'); return error })
    ...
    if (scenario === 'restart') {
      const attempt = active.attemptToPostReqsToNetwork.bind(active)
      active.attemptToPostReqsToNetwork = async () => { throw new Error('stop after durable queue') }
      await assert.rejects(resume)
      ...
      active = new StorageKnex({ chain: 'test', knex: walletDb, commissionSatoshis: 0, feeModel: { model: 'sat/kb', value: 1 } })
      active.setServices(services)
      await active.makeAvailable()
      manager = new WalletStorageManager(identityKey, active)
      await manager.makeAvailable()
      monitor = new Monitor({ chain: 'test', storage: manager, services, chaintracks: services.tracker, startupTaskMode: 'none' })
      monitor.addDefaultTasks()
      wallet = new Wallet({ chain: 'test', keyDeriver: new CachedKeyDeriver(root), storage: manager, services, monitor })
    }
    ...
    if (scenario === 'delayed') {
      const queued = await wallet.createAction({ description: 'resume delayed original action', options: { sendWith: [txid] } })
